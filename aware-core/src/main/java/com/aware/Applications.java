    ...
    public void onAccessibilityEvent(AccessibilityEvent event) {
        ...
        if (Aware.getSetting(getApplicationContext(), Aware_Preferences.STATUS_SCREENTEXT).equals("true") && getScreenStatus() == 0) {
            ...
            if (!currScreenText.equals("") && track_screentext && !event.isPassword()) {
                ...
                int hashedText = currScreenText.hashCode();
                String currPackageName = event.getPackageName().toString();// code for screentext demo

                // Get rid of the duplicate text only if the foreground app is the same
                if (!textBuffer.contains(hashedText) || !currPackageName.equals(prevPackagename)){
                    textBuffer.add(hashedText);
                    contentBuffer.add(screenText);
                    prevPackagename = currPackageName;// code for screentext demo
                    Log.d(TAG, "SCREENTEXT-DEMO (" + currPackageName + ") - " + currScreenText); // code for screentext demo
//                    Log.d(TAG,"Buffer size：" + textBuffer.size());
                }
